      ...
      <Container className={styles.content}>
        <motion.div initial='initial' animate='enter' exit='initial' variants={variants} transition={transition}>
          <motion.div
            ...
            <h3 className={styles.title}>{t('freeCases.title')}</h3>

            <div className={styles.imgWrapper}>
              <div className={styles.img}>
                <img src={pepe} alt='pepe' width={80} height={80} />
                <img className={styles.stars} src='/img/stars-fx.png' alt='stars effect' />
              </div>
              <ClaimBtn />
            </div>

            <p className={styles.description}>{t('freeCases.description')}</p>

            <ProgressBar value={userTickets} max={Number(requiredTickets)} />
          </motion.div>

          <div className={styles.caseWrapper}>
            {(!canPlayFreeCase || !hasFreeCrystals) && !isOpening && (
              <div className={styles.blured}>
                <LockIcon width={40} height={40} className={styles.lockIcon} />
              </div>
            )}

            <Case openingAnimation={STATIC_ANIMATIONS} staticAnimation={firstFreeCrystal?.file_url ?? ''} />

            {hasFreeCrystals && <Open disabled={!canPlayFreeCase} freeCase />}
          </div>
        </motion.div>
      </Container>
      <Modal type='info'>
        <InfoModal title={t('freeCases.info.title')} text={t('freeCases.info.text')} />
      </Modal>
    </>
  );
};
