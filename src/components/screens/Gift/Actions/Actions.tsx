...
const Actions: FC<ActionsProps> = ({ sellPrice }) => {
    ...
    const updateTimer = () => {
      ...
      if (remaining) {
        ...
        if (totalSeconds > 5) {
          ...
          if (remaining.days > 0) {
            const daysWord = lang === 'ru' ? pluralize(remaining.days, ['день', 'дня', 'дней'], 'ru') : pluralize(remaining.days, ['day', 'days'], 'en');
            parts.push(`${remaining.days} ${daysWord}`);
          }

          if (remaining.hours > 0) {
            const hoursWord =
              lang === 'ru' ? pluralize(remaining.hours, ['час', 'часа', 'часов'], 'ru') : pluralize(remaining.hours, ['hour', 'hours'], 'en');
  ...
  const modalTitle = modalType === 'sell' ? t('confirmation.sell.title') : t('confirmation.withdraw.title');

  const modalText = modalType === 'sell' ? t('confirmation.sell.text') : t('confirmation.withdraw.text');
  ...
  return createPortal(
    <>
      <motion.section className={styles.actions} initial='initial' animate='enter' variants={variants} transition={transition}>
                ...
                <div className={styles.wrapper}>
                  ...
                  {isOpen && (
                    <div ref={popupRef} className={styles.popup}>
                      ...
                      <p className={styles.text}>{t('withdrawBubble')}</p>
                    </div>
                  )}
                </div>
              ) : (
      ...
      {modalType === 'timer' ? (
        <Modal type='info'>
          <InfoModal title={t('gift.withdrawTimer.alertTitle')} text={`${t('gift.withdrawTimer.alertText')} ${timerText}.`} />
        </Modal>
      ) : modalType === 'withdrawalsDisabled' ? (
        <Modal type='info'>
          <InfoModal title={t('gift.withdrawalsDisabled.alertTitle')} text={t('gift.withdrawalsDisabled.alertText')} />
        </Modal>
      ) : modalType === 'withdraw' ? (
        <Modal type='confirmation'>
          <WithdrawModal
            ...
            fees={{
              telegram: isDepositGift ? (configData?.withdrawal_fee_direct_telegram ?? 0) : (configData?.withdrawal_fee_template_telegram ?? 0),
              portals: isDepositGift ? (configData?.withdrawal_fee_direct_portals ?? 0) : (configData?.withdrawal_fee_template_portals ?? 0),
            }}
        ...
        <Modal type='confirmation'>
          <ConfirmModal
            title={modalTitle}
            text={modalText}
            fee={isDepositGift ? configData?.sell_fee_direct : configData?.sell_fee_template}
