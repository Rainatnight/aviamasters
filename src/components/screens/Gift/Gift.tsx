...
const Gift: FC = () => {
    ...
    return (
      <motion.div
        ...
        <div className={styles.buttonWrapper}>
          <Button className={styles.button} onClick={() => navigate(fromActivity ? `/collection?tab=activity` : '/')}>
            {t('donate.back')}
          </Button>
        </div>
      </motion.div>
  ...
  const sellPriceFromApi = sellPriceFromApiRaw != null ? Number.parseFloat(String(sellPriceFromApiRaw)) : Number.NaN;

  return (
    <>
      <motion.div initial='initial' animate='enter' exit='initial' variants={variants} transition={transition} className={styles.container}>
        <Card rarity={rarity} file_url={caseItem.file_url} slug={caseItem.id ?? ''} />
        <Head rarity={caseItem.rarity} title={gift?.gift ?? ''} number={extractNumber(caseItem.id ?? '')!} price={caseItem.amount ?? '0'} />
        <Table model={{ text: gift.model ?? undefined }} backdrop={{ text: gift.background ?? undefined }} symbol={{ text: gift.symbol ?? undefined }} />
      </motion.div>
      {(caseItem.can_sell || caseItem.can_withdraw) && !fromActivity ? (
        <Actions
          ...
          sellPrice={
            Number.isFinite(sellPriceFromApi)
              ? sellPriceFromApi
              : salePrice(
                  Number.parseFloat(caseItem.amount ?? '0'),
                  isDepositGift ? (configData?.sell_fee_direct ?? 0) : (configData?.sell_fee_template ?? 0),
                )
          }
        />
      ) : (
