...
const BalanceScreen: FC = () => {
  ...
  return (
    <motion.div initial='initial' animate='enter' exit='initial' variants={variants} transition={transition} className={styles.helpWrapper}>
      <div
        ...
        <div className={styles.balanceLabel}>
          <div className={styles.balanceLabel__value}>
            {t(`donate.balance`)} <span>{infoData?.is_demo ? accountData?.demo_balance : accountData?.balance}</span>
          </div>
        ...
        <div className={styles.formButtons}>
          <CryptoPayComponent
            ...
            customClass={styles.formButtons__item}
            amount={Number(amount)}
            setIsShowInfo={setShowValidate}
          />
          <Button
            secondary
            percent={accountData?.deposit_bonus_percent}
        ...
        <>
          {ReactDOM.createPortal(
            <div
              ...
              style={{
                bottom: isMobile && isPromocodeInputFocused && isKeyboardVisible ? `16px` : '104px',
              }}>
              ...
              {promocodeData && promocodeData.promocode.deposit_bonus && (
                <div className={styles.promoPopup} style={{ bottom: '72px' }}>
                  {t(`donate.promocodes.accept_promo.first`)} <span>{`+${promocodeData?.promocode.deposit_bonus ?? 0}%`}</span>{' '}
              ...
              {promocodeData && promocodeData.promocode.pepes_amount && (
                <div className={styles.promoPopup} style={{ bottom: '72px' }}>
                  ...
                  <span>
                    ...
                    {pluralize(promocodeData.promocode.pepes_amount ?? 0, language === 'ru' ? ['Пепу', 'Пепы', 'Пеп'] : ['Pepe', 'Pepes'], language)}
                  </span>
                </div>
              )}
              ...
              {promocodeData && promocodeData.promocode.ton_amount && (
                <div className={styles.promoPopup} style={{ bottom: '72px' }}>
                  {t(`donate.promocodes.accept_promo.first`)} <span>{promocodeData.promocode.ton_amount ?? 0} TON</span>
                </div>
              )}
          ...
          {ReactDOM.createPortal(
            <div className={cn(styles.footerBack, { [styles.hiddenOnKeyboard]: isKeyboardVisible })} onClick={() => navigate(-1)}>
              {t(`donate.back`)}
            </div>,
