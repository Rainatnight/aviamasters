...
const Open: FC<Props> = ({ freeCase = false, disabled = false }) => {
  ...
  const handleOpenCaseButton = () => {
    ...
    if (
      infoData?.is_demo
        ? accountData?.demo_balance
        : accountData?.balance && Number.parseFloat(currentCrystal.price) <= (infoData?.is_demo ? accountData?.demo_balance : accountData?.balance)
    ) {
      playGame({ case_id: currentCrystal.id });
    } else {
      navigate('/deposit');
  ...
  return createPortal(
    <motion.section className={styles.open} initial='initial' animate='enter' variants={variants} transition={transition}>
      <Container>
        <motion.div initial='initial' animate={isOpening && 'animate'} variants={wrapperVariants}>
          <Button
            className={`${styles.button} ${freeCase ? styles.free : ''} ${disabled ? styles.disabled : ''}`}
            primary
            disabled={isLoading || isOpening || disabled}
            onClick={handleOpenCaseButton}>
