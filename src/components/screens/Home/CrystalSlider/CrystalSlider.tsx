...
export default function CrystalSlider() {
  ...
  const [initialIndex] = useState(() => {
    if (!filteredCrystals.length) return 0;
  ...
  useEffect(() => {
    if (!emblaApi || !filteredCrystals.length) return;
  ...
  useLayoutEffect(() => {
    if (!emblaApi || !filteredCrystals.length || !currentCrystal) return;
  ...
  const scrollWithDelay = useCallback(
    (scrollFn: () => void) => {
      if (!emblaApi || isScrolling) return;
  ...
  return (
    <div className={styles.slider}>
      <div className={cn(styles.embla, isOpening && styles.disabled)} ref={emblaRef}>
        ...
        {!isOpening && (
          <>
            <button type='button' className={styles.prevButton} onClick={scrollPrev} aria-label='Previous slide' disabled={isScrolling}>
              <img src={leftChevron} alt='prev' />
            </button>
            <button type='button' className={styles.nextButton} onClick={scrollNext} aria-label='Next slide' disabled={isScrolling}>
