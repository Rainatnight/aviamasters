...
const Roulette: FC<RouletteProps> = ({ isOpenRoulette }) => {
  const currentCrystal = useSelector((state: RootState) => state.crystals.currentCrystal, shallowEqual);
  ...
  useEffect(() => {
    if (!caseItem || !fullGiftList?.length) return;
