...
const pages = [
    ...
    desc: (
      <>
        <p>
          ...
          <span className={styles.highlight}> Крути кристалл, получай</span> призы и обменивай их на TON. Каждое открытие — шанс на
  ...
  {
    id: 2,
    img: <img src={img2} alt='Magic dust explanation' className={`${styles['onboarding-image']} ${styles['onboarding-image--type2']}`} />,
    title: 'Готов открыть свой первый Кристалл?',
    desc: (
      <p>
        <span className={styles.highlight}>Пополни баланс</span> — ты можешь использовать
        ...
        <motion.div
          ...
          <div className={styles.page}>
            ...
            <div className={styles.dots}>
              {pages.map((_, index) => (
                <div key={index} className={`${styles.dot} ${index === currentPage ? styles['dot--active'] : ''}`} />
              ))}
            </div>
          </div>
        </motion.div>
