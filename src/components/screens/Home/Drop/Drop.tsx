...
const Drop: FC = () => {
  ...
  return (
    <AnimatePresence>
      <motion.section className={styles.drop} initial='animate' animate={isOpening ? 'animate' : 'initial'} exit='animate' variants={wrapperVariants}>
        <Container>
          <div className={styles.head}>
            <motion.button
              ...
              <Icon name='chevron-up' width={20} height={20} className={cn(styles.icon, isShowDropModal && styles.rotate)} />
            </motion.button>
          </div>
          <AnimatePresence mode='wait'>
            {isShowDropModal && (
              <>
                ...
                <motion.div ref={ref} className={styles.body} initial='closed' animate='open' exit='closed' variants={dropdownVariants}>
                  {allRewards.length > 0 &&
