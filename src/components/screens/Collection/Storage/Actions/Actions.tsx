...
const WithdrawActions: FC = () => {
  ...
  const modalTitle = modalType === 'sell' ? t('collection.storage.confirm.sell.title') : t('collection.storage.confirm.withdraw.title');

  const modalText = modalType === 'sell' ? t('collection.storage.confirm.sell.text') : t('collection.storage.confirm.withdraw.text');

  const totalPrice = isGiftsSuccess ? Math.round(giftsData.items.reduce((sum, gift) => sum + Number.parseFloat(gift.sell_price), 0) * 10) / 10 : 0;
  ...
  async function handleActionButton(action: 'sell' | 'withdraw', destination?: 'portals' | 'telegram') {
    ...
    try {
      if (action === 'sell') {
        ...
        await Promise.all(giftIds.map((id) => withdrawGift({ user_gift_id: id, destination }).unwrap()));
      }
    } catch (error) {
      console.error(error);
    }
    ...
    isGiftsSuccess && giftsData.items.length > 0 && infoData?.is_demo !== true && (
      <>
            ...
            <div className={styles.wrapper}>
              ...
              {isOpen && (
                <div ref={popupRef} className={styles.popup}>
                  ...
                  <p className={styles.text}>{destination === 'portals' ? t('withdrawBubble') : t('withdrawBubbleTelegram')}</p>
                </div>
              )}
            </div>
          </Container>
        ...
        {modalType === 'withdrawalsDisabled' ? (
          ...
          <Modal type='confirmation'>
            <ConfirmModal title={modalTitle} text={modalText} onConfirm={handleConfirm} onClose={handleClose} />
          </Modal>
        )}
      </>
    ),
