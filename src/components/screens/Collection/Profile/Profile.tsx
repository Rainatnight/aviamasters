      ...
      <Container>
        <div className={styles.user}>
          <div className={styles.avatar}>
            <img className={styles.img} src={infoData?.pfp_url} alt={infoData?.username} />
            {leaderboardData?.current_user?.place && (
              <div className={styles.badge}>{isLeaderboardSuccess && formatNumber(leaderboardData?.current_user?.place)}</div>
            )}
          </div>
          <h3 className={styles.username}>{` ${infoData?.first_name} ${infoData?.last_name}`}</h3>
          <WalletConnect />
        </div>
      </Container>
    </section>
