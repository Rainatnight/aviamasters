...
const Actions: FC = () => {
  ...
  const { data: actionsData, isSuccess: isActionsSuccess, isFetching: isActionsFetching } = useGetSpinsQuery({ page, demo: false, size: 50 });
  ...
  return (
    <motion.div className={styles.actions} initial='initial' animate='enter' variants={variants} transition={transition}>
      <h1 className={styles.title}>{t('collection.actions.title')}</h1>
      <div className={styles.body}>
        {isActionsSuccess &&
          filteredItems.map((item) => (
            <Item action='won_prize' spinItem={item} date={new Date(item?.date).toLocaleDateString()} key={item.date + item.name} />
          ))}
        <div ref={sentinelRef} />
      </div>
    </motion.div>
