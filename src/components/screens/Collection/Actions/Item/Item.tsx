    ...
    <div
      ...
      onClick={() => {
        if (spinItem.prize_type === PrizeTypeEnum.PROMOCODE) {
          ...
          setCase({
            ...spinItem,
            type: PrizeTypeEnum.GIFT,
            id: spinItem.date,
            can_sell: false,
            can_withdraw: false,
            amount: spinItem.price.toString(),
      ...
      <div className={styles.offer}>
        <h3 className={styles.title}>
          ...
          <span style={{ color: `var(--color-rarity-${rarityLowered})` }}>{spinItem.name}</span>
        </h3>
        <div className={styles.date}>{date}</div>
      </div>
      <div className={styles.action}>
        {action === 'won_prize' && <div className={styles.win}>+1</div>}
        {action === 'sale' && coins && (
        ...
        {action === 'withdraw' && <Icon name='withdraw' width={16} height={16} />}
      </div>
    </div>
  );
};
