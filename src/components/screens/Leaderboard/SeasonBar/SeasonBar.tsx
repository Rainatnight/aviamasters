...
function SeasonBar() {
  ...
  return (
    <div className={styles.tabs}>
      <Button
        className={`${styles.tab} ${selectedSeason?.kind === 'all_time' ? styles.active : ''}`}
        onClick={() => {
          dispatch(selectSeason({ kind: 'all_time' }));
        }}>
      ...
      {seasons?.seasons.slice(0, 2).map((season) => (
        <button
          type='button'
          key={season.id}
          className={`${styles.tab} ${selectedSeason?.id === season.id ? styles.active : ''}`}
          onClick={() => {
      ...
      <Button className={styles.tab} secondary onClick={() => dispatch(openModal())}>
        See All
      </Button>
    </div>
  );
