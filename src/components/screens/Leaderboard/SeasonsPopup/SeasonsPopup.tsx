...
function SeasonsPopup() {
  ...
  if (!modalOpen || !leaderboardData) return null;
    ...
    <motion.div
      ...
      <div>
        ...
        <div className={styles.grid}>
          {leaderboardData.seasons &&
            leaderboardData.seasons?.map((season) => (
              <button
                type='button'
                key={season.id}
                ...
                <span className={styles.season}>Season {season.id}</span>
                <span className={styles.date}>{season.name}</span>
              </button>
            ))}
        </div>
      </div>
