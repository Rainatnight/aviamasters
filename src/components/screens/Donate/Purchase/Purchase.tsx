...
const Purchase: FC = () => {
  ...
  useEffect(() => {
    const fn = currency === 'stars' ? convertToStars : convertToTon;
    Promise.all(options.map((opt) => fn({ amount: opt }).unwrap())).then((results) => setOptionPrices(results.map((r) => r.output_amount)));
      ...
      <Container>
        ...
        <div className={styles.options}>
          {options.map((purchasedAmount, idx) => (
            <div className={styles.option} onClick={() => handleCardSelect(purchasedAmount)} key={purchasedAmount}>
              <OptionCard paymentCurrency={currency} purchasedAmount={purchasedAmount} paymentAmount={optionPrices[idx]} />
            </div>
          ))}
        </div>
      </Container>
