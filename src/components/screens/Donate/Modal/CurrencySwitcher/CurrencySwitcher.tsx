...
export default function CurrencySwitcher({ currency, percent, setCurrency }: CurrencySwitcherProps) {
  return (
    <div className={styles.switcher}>
      {percent && percent > 1 ? <div className={styles.salePlank}>{percent.toFixed(0)}%</div> : null}
      <button type='button' className={`${styles.iconButton} ${currency === 'ton' ? styles.active : styles.inactive}`} onClick={() => setCurrency('ton')}>
