...
export const profileApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getSpins: builder.query<SpinsResponse, { page?: number; demo?: boolean; size?: number }>({
      ...
      serializeQueryArgs: ({ endpointName, queryArgs }) => `${endpointName}-${queryArgs.demo ? 'demo' : 'prod'}-${queryArgs.size ?? 10}`,
      ...
      forceRefetch: ({ currentArg, previousArg }) =>
        currentArg?.page !== previousArg?.page || currentArg?.demo !== previousArg?.demo || currentArg?.size !== previousArg?.size,
      providesTags: ['Spins'],
    }),
